...
5. **Images — closings done, listings/land still open.** Team headshots (all 5) and photos for all 61 closed sales (7 of which also appear on their case study page) are in place under `public/images/`, wired into `src/data/*.ts` via `image`/`images`/`headshot` fields, and rendered with descriptive alt text. Still outstanding: active listings (none exist yet — no photos needed until there's inventory) and a real (non-stock) land parcel photo for Land for Development (currently a generic placeholder). Formats are as-received (JPG/PNG) — not yet converted to WebP/compressed for performance.
