...
5. **Images — done.** Team headshots, photos for all 61 closed sales, and photos for both active listings (333 N Union Ave, 3815 Don Tomaso Dr) are in place under `public/images/`, wired into `src/data/*.ts` via `image`/`images`/`headshot` fields, and rendered with descriptive alt text. All are now WebP and compressed for performance. Still outstanding: a real (non-stock) land parcel photo for Land for Development (currently a generic placeholder).
...
7. **URL redirects before DNS cutover.** The live WordPress site (`highlightreg.com`) has several URLs that don't match this site's structure: `/project/*` → `/case-studies/*-{submarket}-apartments/`, `/success-stories/` → `/testimonials/`, `/closed-sales/` already matches, `/privacy-policy-2/` → `/privacy-policy/`, and `/josh-kaplan/`/`/troy-lucero/` have no direct equivalent (bios are inline on `/about/`). Build 301 redirects for these in `worker/index.ts` before pointing DNS at this Worker, to avoid losing indexed search rankings and breaking existing links.
